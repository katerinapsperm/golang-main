...
func (handler *CarsHandler) Create(w http.ResponseWriter, r *http.Request) { // все методы хендлеров имеют одинаковую сигнатуру - две переменных и ничего в ответ
	...
	err = handler.processor.CreateCar(r.Context(), newCar) // вызываем метод процессора и отрабатываем ошибки
	if err != nil {
		WrapError(w, err)
		return
	}
...
func (handler *CarsHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	list, err := handler.processor.ListCars(r.Context(), userIdFilter, strings.Trim(vars.Get("brand"), "\""), strings.Trim(vars.Get("colour"), "\""), strings.Trim(vars.Get("license_plate"), "\""))
...
func (handler *CarsHandler) Find(w http.ResponseWriter, r *http.Request) {
	...
	car, err := handler.processor.FindCar(r.Context(), id)
